...
if __name__ == '__main__':
    ...
    cost, Time, energy = np.zeros((MonteCarlo, len(beta_set))), np.zeros((MonteCarlo, len(beta_set))), np.zeros((MonteCarlo, len(beta_set)))
    ...
    for idx, beta_t in enumerate(beta_set):
        ...
        channel = np.loadtxt('../data/K{0}_D{1}/InputData_MonteCarlo{2}_K{0}_Distance{1}.txt'.format(str(K), str(Distance), str(channel_num)))

        for cyc in range(MonteCarlo):
            lr = LR(beta_v)
            cost[cyc, idx], Time[cyc, idx], energy[cyc, idx], decision = lr.optimize(channel[cyc, :])

            if cyc % (MonteCarlo//10) == 0:
                print('episode:', cyc, cost[cyc, idx], decision, lr.beta*Time[cyc, idx]+energy[cyc, idx])
    ...
    np.savetxt('../data/beta/Binary_LR_beta_VS_cost_MonteCarlo{0}_K{1}_Distance{2}_add.txt'.format(str(MonteCarlo), str(K),str(Distance)), cost)
    np.savetxt('../data/beta/Binary_LR_beta_VS_energy_MonteCarlo{0}_K{1}_Distance{2}_add.txt'.format(str(MonteCarlo), str(K),str(Distance)), energy)
    np.savetxt('../data/beta/Binary_LR_beta_VS_Time_MonteCarlo{0}_K{1}_Distance{2}_add.txt'.format(str(MonteCarlo), str(K),str(Distance)), Time)
